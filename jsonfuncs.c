...
static Datum get_jsonbc_path_all(FunctionCallInfo fcinfo, bool as_text);
...
static JsonbcValue *findJsonbcValueFromContainerLen(JsonbcContainer *container,
							   uint32 flags,
							   char *key,
							   uint32 keylen);
 ...
 */
Datum
jsonbc_object_keys(PG_FUNCTION_ARGS)
{
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		while ((r = JsonbcIteratorNext(&it, &v, skipNested)) != WJB_DONE)
		{
			skipNested = true;
...
Datum
jsonbc_object_field(PG_FUNCTION_ARGS)
{
	Jsonbc	   *jb = PG_GETARG_JSONB(0);
	...
	v = findJsonbcValueFromContainerLen(&jb->root, JB_FOBJECT,
									   VARDATA_ANY(key),
									   VARSIZE_ANY_EXHDR(key));

	if (v != NULL)
		PG_RETURN_JSONB(JsonbcValueToJsonbc(v));
...
Datum
jsonbc_object_field_text(PG_FUNCTION_ARGS)
{
	Jsonbc	   *jb = PG_GETARG_JSONB(0);
	text	   *key = PG_GETARG_TEXT_PP(1);
	JsonbcValue *v;
			...
			case jbvBinary:
				{
					...
					(void) JsonbcToCString(jtext, v->val.binary.data, -1);
...
Datum
jsonbc_array_element(PG_FUNCTION_ARGS)
{
	Jsonbc	   *jb = PG_GETARG_JSONB(0);
	int			element = PG_GETARG_INT32(1);
	JsonbcValue *v;
	...
	v = getIthJsonbcValueFromContainer(&jb->root, element);
	if (v != NULL)
		PG_RETURN_JSONB(JsonbcValueToJsonbc(v));
...
Datum
jsonbc_array_element_text(PG_FUNCTION_ARGS)
{
	Jsonbc	   *jb = PG_GETARG_JSONB(0);
	int			element = PG_GETARG_INT32(1);
	JsonbcValue *v;
...
Datum
jsonbc_extract_path(PG_FUNCTION_ARGS)
{
	return get_jsonbc_path_all(fcinfo, false);
}

Datum
jsonbc_extract_path_text(PG_FUNCTION_ARGS)
{
	return get_jsonbc_path_all(fcinfo, true);
}

static Datum
get_jsonbc_path_all(FunctionCallInfo fcinfo, bool as_text)
{
	Jsonbc	   *jb = PG_GETARG_JSONB(0);
	ArrayType  *path = PG_GETARG_ARRAYTYPE_P(1);
	Jsonbc	   *res;
	...
	else
	{
		...
		if (npath <= 0)
			jbvp = getIthJsonbcValueFromContainer(container, 0);
	}
	 ...
	 * JsonbcValue directly for root-level containers.)
	 */
	if (npath <= 0 && jbvp == NULL)
	{
		if (as_text)
		{
			PG_RETURN_TEXT_P(cstring_to_text(JsonbcToCString(NULL,
															container,
															VARSIZE(jb))));
		}
		else
	...
	for (i = 0; i < npath; i++)
	{
		if (have_object)
		{
			jbvp = findJsonbcValueFromContainerLen(container,
												  JB_FOBJECT,
		...
		else if (have_array)
		{
			...
			if (endptr == indextext || *endptr != '\0' || errno != 0 ||
				...
				PG_RETURN_NULL();
			index = (uint32) lindex;
			jbvp = getIthJsonbcValueFromContainer(container, index);
		}
		else
		{
		...
		{
			JsonbcIterator *it = JsonbcIteratorInit((JsonbcContainer *) jbvp->val.binary.data);
			int			r;

			r = JsonbcIteratorNext(&it, &tv, true);
			container = (JsonbcContainer *) jbvp->val.binary.data;
...
Datum
jsonbc_array_length(PG_FUNCTION_ARGS)
{
	Jsonbc	   *jb = PG_GETARG_JSONB(0);
...
each_worker_jsonb(FunctionCallInfo fcinfo, const char *funcname, bool as_text)
{
	...
	while ((r = JsonbcIteratorNext(&it, &v, skipNested)) != WJB_DONE)
	{
		...
		if (r == WJB_KEY)
		{
			 ...
			 */
			r = JsonbcIteratorNext(&it, &v, skipNested);
			...
			if (as_text)
			{
				...
				else
				{
					...
					else
					{
						...
						StringInfo	jtext = makeStringInfo();
						Jsonbc	   *jb = JsonbcValueToJsonbc(&v);

						(void) JsonbcToCString(jtext, &jb->root, 0);
			...
			{
				/* Not in text mode, just return the Jsonbc */
				Jsonbc	   *val = JsonbcValueToJsonbc(&v);
					  ...
					  bool as_text)
{
	...
	tmp_cxt = AllocSetContextCreate(CurrentMemoryContext,
									"jsonbc_array_elements temporary cxt",
									ALLOCSET_DEFAULT_MINSIZE,
									ALLOCSET_DEFAULT_INITSIZE,
									ALLOCSET_DEFAULT_MAXSIZE);
 ...
 */
Datum
jsonbc_populate_record(PG_FUNCTION_ARGS)
{
	return populate_record_worker(fcinfo, "jsonbc_populate_record", true);
}

Datum
jsonbc_to_record(PG_FUNCTION_ARGS)
{
	return populate_record_worker(fcinfo, "jsonbc_to_record", false);
}
					   ...
					   bool have_record_arg)
{
	...
	for (i = 0; i < ncolumns; ++i)
	{
		...
		else
		{
			...
			v = findJsonbcValueFromContainerLen(&jb->root, JB_FOBJECT, key,
											   strlen(key));
		}
 ...
 */
Datum
jsonbc_populate_recordset(PG_FUNCTION_ARGS)
{
	return populate_recordset_worker(fcinfo, "jsonbc_populate_recordset", true);
}

Datum
jsonbc_to_recordset(PG_FUNCTION_ARGS)
{
	return populate_recordset_worker(fcinfo, "jsonbc_to_recordset", false);
}
...
make_row_from_rec_and_jsonb(Jsonbc *element, PopulateRecordsetState *state)
{
	...
	for (i = 0; i < ncolumns; ++i)
	{
		...
		v = findJsonbcValueFromContainerLen(&element->root, JB_FOBJECT,
										   key, strlen(key));
						  ...
						  bool have_record_arg)
{
	...
	{
		Jsonbc	   *jb = PG_GETARG_JSONB(json_arg_num);
		JsonbcIterator *it;
		JsonbcValue	v;
		bool		skipNested = false;
		int			r;
		...
		while ((r = JsonbcIteratorNext(&it, &v, skipNested)) != WJB_DONE)
		{
			...
			if (r == WJB_ELEM)
			{
				Jsonbc	   *element = JsonbcValueToJsonbc(&v);
...
/*
 * findJsonbcValueFromContainer() wrapper that sets up JsonbcValue key string.
 */
static JsonbcValue *
findJsonbcValueFromContainerLen(JsonbcContainer *container, uint32 flags,
							   char *key, uint32 keylen)
{
	...
	return findJsonbcValueFromContainer(container, flags, &k);
}
