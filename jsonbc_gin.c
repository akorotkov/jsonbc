 ...
 *
 * jsonbc_gin.c
 *	 GIN support functions for jsonbc
 *
 * Copyright (c) 2014, PostgreSQL Global Development Group
 *
 *
 * IDENTIFICATION
 *	  src/backend/utils/adt/jsonbc_gin.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
static Datum make_scalar_key(const JsonbcValue *scalarVal, bool is_key);

/*
 *
 * jsonbc_ops GIN opclass support functions
 *
 */

Datum
gin_compare_jsonbc(PG_FUNCTION_ARGS)
{
...
Datum
gin_extract_jsonbc(PG_FUNCTION_ARGS)
{
	Jsonbc	   *jb = (Jsonbc *) PG_GETARG_JSONB(0);
	...
	while ((r = JsonbcIteratorNext(&it, &v, false)) != WJB_DONE)
	{
		...
		switch (r)
		{
			case WJB_KEY:
				...
				break;
			case WJB_ELEM:
				/* Pretend string array elements are keys, see jsonbc.h */
...
Datum
gin_extract_jsonbc_query(PG_FUNCTION_ARGS)
{
	int32	   *nentries = (int32 *) PG_GETARG_POINTER(1);
	...
	if (strategy == JsonbcContainsStrategyNumber)
	{
		/* Query is a jsonbc, so just apply gin_extract_jsonbc... */
		entries = (Datum *)
			DatumGetPointer(DirectFunctionCall2(gin_extract_jsonbc,
												PG_GETARG_DATUM(0),
	...
	else if (strategy == JsonbcExistsStrategyNumber)
	{
	...
	else if (strategy == JsonbcExistsAnyStrategyNumber ||
			 strategy == JsonbcExistsAllStrategyNumber)
	{
		...
		*nentries = j;
		...
		if (j == 0 && strategy == JsonbcExistsAllStrategyNumber)
			*searchMode = GIN_SEARCH_MODE_ALL;
	}
	else
	{
...
Datum
gin_consistent_jsonbc(PG_FUNCTION_ARGS)
{
	bool	   *check = (bool *) PG_GETARG_POINTER(0);
	StrategyNumber strategy = PG_GETARG_UINT16(1);

	/* Jsonbc	   *query = PG_GETARG_JSONB(2); */
	...
	else if (strategy == JsonbcExistsAllStrategyNumber)
	{
		/* As for plain exists, we must recheck */
		*recheck = true;
...
Datum
gin_triconsistent_jsonbc(PG_FUNCTION_ARGS)
{
		...
		strategy == JsonbcExistsAllStrategyNumber)
	{
		...
		for (i = 0; i < nkeys; i++)
		{
			if (check[i] == GIN_FALSE)
			{
			 ...
			 strategy == JsonbcExistsAnyStrategyNumber)
	{
...
/*
 *
 * jsonbc_path_ops GIN opclass support functions
 *
 * In a jsonbc_path_ops index, the GIN keys are uint32 hashes, one per JSON
...
Datum
gin_extract_jsonbc_path(PG_FUNCTION_ARGS)
{
		...
		switch (r)
		{
			...
			case WJB_BEGIN_OBJECT:
				...
				else
				{
					 ...
					 * are that in the base jsonbc object storage already.
					 */
			...
			case WJB_KEY:
				...
				/* and mix in this key */
				JsonbcHashScalarValue(&v, &stack->hash);
...
Datum
gin_extract_jsonbc_query_path(PG_FUNCTION_ARGS)
{
	int32	   *nentries = (int32 *) PG_GETARG_POINTER(1);
	StrategyNumber strategy = PG_GETARG_UINT16(2);
	int32	   *searchMode = (int32 *) PG_GETARG_POINTER(6);
	...
	/* Query is a jsonbc, so just apply gin_extract_jsonbc_path ... */
	entries = (Datum *)
		DatumGetPointer(DirectFunctionCall2(gin_extract_jsonbc_path,
											PG_GETARG_DATUM(0),
											PointerGetDatum(nentries)));
...
Datum
gin_consistent_jsonbc_path(PG_FUNCTION_ARGS)
{
	bool	   *check = (bool *) PG_GETARG_POINTER(0);
	StrategyNumber strategy = PG_GETARG_UINT16(1);
	...
	/*
	 * jsonbc_path_ops is necessarily lossy, not only because of hash
...
Datum
gin_triconsistent_jsonbc_path(PG_FUNCTION_ARGS)
{
	GinTernaryValue *check = (GinTernaryValue *) PG_GETARG_POINTER(0);
	StrategyNumber strategy = PG_GETARG_UINT16(1);
...
/*
 * Construct a jsonbc_ops GIN key from a flag byte and a textual representation
...
/*
 * Create a textual representation of a JsonbcValue that will serve as a GIN
 * key in a jsonbc_ops index.  is_key is true if the JsonbcValue is a key,
 * or if it is a string array element (since we pretend those are keys,
 * see jsonbc.h).
 */
static Datum
make_scalar_key(const JsonbcValue *scalarVal, bool is_key)
{
	Datum		item;
	char	   *cstr;
	...
	switch (scalarVal->type)
	{
		...
		case jbvNumeric:
			 ...
			 * storing a "union" type in the GIN B-Tree, and indexing Jsonbc
		...
		case jbvString:
			...
			break;
		default:
			elog(ERROR, "unrecognized jsonbc scalar type: %d", scalarVal->type);
